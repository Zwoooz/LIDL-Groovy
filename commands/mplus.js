...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const mPlus = async (name, nameServer, nameRegion) => {
      const res = await fetch(`https://raider.io/api/v1/characters/profile?region=${nameRegion}&realm=${nameServer}&name=${name}&fields=mythic_plus_scores`);
      ...
      const mPlusEmbed = new EmbedBuilder()
        ...
        .addFields(
          { name: 'Mythic+ Score:', value: `**${user.mythic_plus_scores.all}**`, inline: true },
        );
